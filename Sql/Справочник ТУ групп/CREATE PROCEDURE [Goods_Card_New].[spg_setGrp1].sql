...
-- =============================================
CREATE PROCEDURE [Goods_Card_New].[spg_setGrp1]			 
	@id int,
	@cName varchar(max),	
	@id_otdel int,
	@id_nds  int,
...
BEGIN TRY 
	IF @isDel = 0
		BEGIN
			...
			IF EXISTS (select TOP(1) id from [dbo].[s_grp1] where id <>@id and LTRIM(RTRIM(LOWER([cname]))) = LTRIM(RTRIM(LOWER(@cName))) && id_otdel = @id_otdel)
				BEGIN
					SELECT -1 as id;
					return;
				END
